...
class YouTubeService: YouTubeServiceProtocol {
    ...
    private func executeQuery<T: GTLRObject>(_ query: GTLRQuery) async throws -> T {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            // Use [weak self] in the completion handler to break the retain cycle:
            // service -> GTLRServiceTicket -> completion handler -> self
            service.executeQuery(query) { [weak self] (ticket: GTLRServiceTicket, response: Any?, error: Error?) in
                if let error = error {
                    // Use optional chaining to gracefully handle deallocated self
                    let mappedError = self?.mapError(error) ?? YouTubeError.unknownError
                ...
                // Task wrapper required by Swift 6 strict concurrency to safely
                // transfer response across actor boundaries (GTLRObject -> MainActor)
                Task { @MainActor in
